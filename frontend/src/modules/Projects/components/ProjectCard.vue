<template>
  <article class="project-card">
    ...
    <div class="project-card__image">
      ...
      >
        <HeartIcon 
          :class="['heart-icon', { 'heart-favorite': project.isFavorite }]"
        />
      </button>
    </div>
    ...
    <div class="project-card__content">
      ...
      <div class="title-section">
        ...
        <div class="location-info">
          ...
          <span class="location-text">{{ project.location }}{{ project.country ? `, ${project.country}` : '' }}</span>
        </div>
      </div>
      ...
      <div class="price-grid">
        <div class="price-item">
          ...
          <div class="price-value price-value-bold">{{ formatCurrency(project.price) }}</div>
        </div>
        <div class="price-item">
          <div class="price-label">Token Price</div>
          <div class="price-value">{{ formatCurrency(project.tokenPrice || project.pricePerToken) }}</div>
        </div>
        <div class="price-item">
          <div class="price-label">Min Investment</div>
          <div class="price-value">{{ formatCurrency(project.minInvestment) }}</div>
        </div>
        <div class="price-item">
          <div class="price-label">IRR</div>
          <div class="price-value irr-value">
            {{ project.irr === 'Request' ? 'Request' : formatPercentage(project.irr) }}
          </div>
        </div>
        <div class="price-item">
          <div class="price-label">APR</div>
          <div class="price-value">{{ formatPercentage(project.apr) }}</div>
        </div>
        <div class="price-item">
          <div class="price-label">Value Growth</div>
          <div class="price-value">{{ formatPercentage(project.valueGrowth) }}</div>
        </div>
      </div>
      ...
      <div class="visit-stats">
        <div class="visit-item">
          ...
          <span class="visit-text">Visited {{ project.visitsThisWeek || 0 }} times this week</span>
        </div>
        <div class="visit-total">
          {{ project.totalVisitors || 0 }} all time visitors
        </div>
      </div>
...
const displayImage = computed(() => {
  if (props.project.imageUrls && props.project.imageUrls.length > 0) {
    return props.project.imageUrls[currentImageIndex.value]
  }
  if (typeof props.project.projectImage === 'string') {
    return props.project.projectImage
  }
...
// Methods
const previousImage = () => {
  if (props.project.imageUrls && props.project.imageUrls.length > 1) {
    currentImageIndex.value = currentImageIndex.value === 0 
      ? props.project.imageUrls.length - 1 
...
const nextImage = () => {
  if (props.project.imageUrls && props.project.imageUrls.length > 1) {
    currentImageIndex.value = currentImageIndex.value === props.project.imageUrls.length - 1 
      ? 0 
      : currentImageIndex.value + 1
  }
}
