...
export class ClientService {
     ...
     */
    async applyAsClient(userId, applicationData) {
        try {
     ...
     */
    async updateClient(userId, updateData) {
        try {
     ...
     */
    async getAllClients(limit = 50, offset = 0, status) {
        try {
     ...
     */
    async canUserApplyAsClient(userId) {
        try {
            const user = await this.prisma.user.findUnique({
                where: { id: userId },
                include: { client: true }
     ...
     */
    async getClientsByStatus(status, limit = 50, offset = 0) {
        try {
            const clients = await this.prisma.client.findMany({
                where: { status },
                orderBy: { createdAt: 'desc' },
