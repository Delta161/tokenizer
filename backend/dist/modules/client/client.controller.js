...
import { validateClientApplication, validateClientUpdate, validateClientStatusUpdate, validateClientIdParam, validateClientListQuery } from './validators/clientSchema.js';
export class ClientController {
     ...
     */
    updateCurrentClientProfile = async (req, res, next) => {
        try {
            if (!req.user) {
                res.status(401).json({
                    success: false,
     ...
     */
    getClientProfileById = async (req, res, next) => {
        try {
            if (!req.user) {
                res.status(401).json({
                    success: false,
