...
import { propertyCreateSchema, propertyUpdateSchema, propertyStatusUpdateSchema } from './property.validators.js';
export class PropertyController {
    ...
    getByIdIfApproved = async (req, res, next) => {
        try {
            const property = await this.getService().getByIdIfApproved(req.params.id);
            if (!property)
                return res.status(404).json({ success: false, error: 'NotFound', message: 'Property not found' });
    ...
    getMyProperties = async (req, res, next) => {
        try {
            const clientId = req.user?.client?.id;
            if (!clientId)
                return res.status(403).json({ success: false, error: 'Forbidden', message: 'Not a client' });
    ...
    create = async (req, res, next) => {
        try {
            ...
            if (!clientId)
                return res.status(403).json({ success: false, error: 'Forbidden', message: 'Not a client' });
            const property = await this.getService().create(clientId, parse.data);
    ...
    update = async (req, res, next) => {
        try {
            ...
            if (!clientId)
                return res.status(403).json({ success: false, error: 'Forbidden', message: 'Not a client' });
            const updated = await this.getService().update(clientId, req.params.id, parse.data);
            if (!updated)
                return res.status(404).json({ success: false, error: 'NotFound', message: 'Property not found or not editable' });
    ...
    delete = async (req, res, next) => {
        try {
            ...
            if (!deleted)
                return res.status(404).json({ success: false, error: 'NotFound', message: 'Property not found or not deletable' });
