...
async function testKycProviderModule() {
    ...
    try {
        ...
        const session = await kycProviderService.initVerification(userId, KycProvider.SUMSUB, 'https://example.com/callback');
        ...
        const status = await kycProviderService.getVerificationStatus(session.referenceId, KycProvider.SUMSUB);
        ...
        const headers = {
            'x-payload-digest': verifySumsubWebhookSignature({}, // Empty headers for test
            mockPayload).payload?.referenceId ? 'valid-signature' : 'invalid-signature'
        };
        const verificationResult = kycProviderService.verifyWebhookSignature(KycProvider.SUMSUB, headers, mockPayload);
        ...
        if (verificationResult.isValid && verificationResult.payload) {
            ...
            // Process webhook
            const updatedRecord = await kycProviderService.processWebhook(KycProvider.SUMSUB, verificationResult.payload);
...
//# sourceMappingURL=kycProvider.test.js.map