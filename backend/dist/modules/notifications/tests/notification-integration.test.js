 ...
 */
import { NotificationService, NotificationTrigger, NotificationDispatcherService, InAppNotificationChannel, EmailNotificationChannel, NotificationType } from '../index';
// Mock PrismaClient
const mockPrisma = {
    notification: {
...
describe('Notification System Integration', () => {
    ...
    beforeEach(() => {
        ...
        notificationTrigger.dispatcher = dispatcherService;
    });
    afterAll(() => {
    ...
    test('should create and dispatch a notification through the trigger', async () => {
        // Act
        const notificationId = await notificationTrigger.triggerNotification('user-123', NotificationType.SYSTEM, 'Test Notification', 'This is a test notification', { testData: 'test-value' });
        // Assert
        ...
        expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Attempting to deliver notification'));
    });
    test('should broadcast a notification to all users', async () => {
        ...
        // Act
        const result = await notificationTrigger.triggerBroadcast('admin-456', NotificationType.ANNOUNCEMENT, 'Broadcast Test', 'This is a broadcast test', { importance: 'high' });
        // Assert
    ...
    test('should handle errors gracefully when user does not exist', async () => {
        // Act
        const notificationId = await notificationTrigger.triggerNotification('non-existent-user', NotificationType.SYSTEM, 'Test Notification', 'This is a test notification');
        // Assert
        expect(notificationId).toBe('notification-123');
        expect(mockPrisma.notification.create).toHaveBeenCalled();
        // Verify error handling (check logs)
        expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Error dispatching notification'), expect.any(Error));
    });
});
//# sourceMappingURL=notification-integration.test.js.map