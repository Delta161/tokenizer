import { KycSubmissionSchema, KycUpdateSchema } from './kyc.validators';
...
export class KycController {
     ...
     */
    getCurrentUserKyc = async (req, res, next) => {
        try {
            if (!req.user?.id) {
                res.status(401).json({
                    success: false,
     ...
     */
    initiateProviderVerification = async (req, res, next) => {
        try {
            if (!req.user?.id) {
                res.status(401).json({
                    success: false,
            ...
            const session = await this.kycService.initiateProviderVerification(req.user.id, provider, redirectUrl);
            res.status(200).json({
                success: true,
                data: {
     ...
     */
    syncKycStatus = async (req, res, next) => {
        try {
    ...
    submitKyc = async (req, res, next) => {
        try {
            ...
            const kycRecord = await this.kycService.submitKyc(req.user.id, validationResult.data);
            res.status(201).json({
                success: true,
                data: kycRecord
            });
     ...
     */
    getAllKycRecords = async (req, res, next) => {
        try {
    ...
    updateKycStatus = async (req, res, next) => {
        try {
            ...
            const kycRecord = await this.kycService.updateKycStatus(userId, validationResult.data);
            res.json({
                success: true,
                data: kycRecord
            });
