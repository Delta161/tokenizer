import { PropertyStatus, ClientStatus } from '@prisma/client';
...
export class ProjectService {
    ...
    async listProjects(options) {
        const { page = 1, limit = 10, status, clientId, clientStatus, hasToken } = options;
        ...
        const projects = filteredProperties.map(property => {
            return mapToProjectDTO(property, property.client, property.tokens && property.tokens.length > 0 ? property.tokens[0] : undefined);
        });
    ...
    async getProjectById(id) {
        ...
        // Map to ProjectDTO format using the mapper utility
        const project = mapToProjectDTO(property, property.client, property.tokens && property.tokens.length > 0 ? property.tokens[0] : undefined);
        return project;
    }
    /**
    ...
    async getFeaturedProjects(limit = 3) {
        ...
        // Filter out properties where client doesn't match the filter
        const filteredProperties = properties.filter(property => property.client && property.client.status === ClientStatus.APPROVED);
        // Map to ProjectDTO format using the mapper utility
        const projects = filteredProperties.map(property => {
            return mapToProjectDTO(property, property.client, property.tokens && property.tokens.length > 0 ? property.tokens[0] : undefined);
        });
