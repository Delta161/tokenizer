...
import { validateClientApplication, validateClientUpdate, validateClientStatusUpdate, validateClientIdParam, validateClientListQuery } from '../validators/client.validators';
...
export class ClientController {
     ...
     */
    applyAsClient = async (req, res, next) => {
        try {
            if (!req.user) {
                res.status(401).json({
                    success: false,
     ...
     */
    getCurrentClientProfile = async (req, res, next) => {
        try {
            if (!req.user) {
                res.status(401).json({
                    success: false,
    ...
    updateClientStatus = async (req, res, next) => {
        try {
            ...
            const updatedClient = await this.clientService.updateClientStatus(idValidation.data.id, statusValidation.data.status);
            res.status(200).json({
                success: true,
                data: updatedClient,
