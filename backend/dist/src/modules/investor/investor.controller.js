...
import { parseInvestorApplication, parseInvestorUpdate, parseInvestorVerificationUpdate, parseWalletCreate, parseWalletVerificationUpdate, parseInvestorIdParam, parseWalletIdParam, parseInvestorListQuery } from './investor.validators.js';
export class InvestorController {
    ...
    updateInvestorVerification = async (req, res) => {
        try {
            ...
            const investor = await this.getService().updateInvestorVerification(parseIdResult.data.id, parseBodyResult.data);
            res.status(200).json({
                success: true,
                data: investor,
    ...
    updateWalletVerification = async (req, res) => {
        try {
            ...
            const wallet = await this.getService().updateWalletVerification(parseIdResult.data.walletId, parseBodyResult.data);
            res.status(200).json({
                success: true,
                data: wallet,
                message: parseBodyResult.data.isVerified
