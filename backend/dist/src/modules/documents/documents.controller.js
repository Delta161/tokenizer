...
export class DocumentController {
    ...
    async uploadDocument(req, res) {
        try {
            ...
            // Upload document
            const document = await this.documentService.uploadDocument(req.user.id, propertyId, req.file);
            // Return document DTO
     ...
     * @param res - Express response
     */
    async getDocumentsByPropertyId(req, res) {
        try {
            // Ensure user is authenticated
            if (!req.user?.id) {
                res.status(401).json({ message: 'Unauthorized' });
    ...
    async getDocumentsByUserId(req, res) {
        try {
            ...
            // Get documents
            const documents = await this.documentService.getDocumentsByUserId(req.user.id, targetUserId);
            // Return document DTOs
            res.status(200).json(documents.map(doc => mapDocumentToDto(doc, req)));
        }
        catch (error) {
    ...
    async listDocuments(req, res) {
        try {
            ...
            // Get documents
            const { documents, total } = await this.documentService.listDocuments(req.user.id, validationResult.data);
            // Return paginated document DTOs
            res.status(200).json(mapToPaginatedDocumentsDto(documents.map(doc => mapDocumentToDto(doc, req)), total, validationResult.data.page || 1, validationResult.data.limit || 10));
        }
        catch (error) {
    ...
    async restoreDocument(req, res) {
        try {
            ...
            // Restore document
            await this.documentService.restoreDocument(validationResult.data.id, req.user.id);
            // Return success response
