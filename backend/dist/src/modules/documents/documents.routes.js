 ...
 */
export function createDocumentsRouter(prisma) {
    const router = express.Router();
    const documentController = new DocumentController(prisma);
    // Apply authentication middleware to all routes
    router.use(requireAuth);
    // Upload document
    router.post('/upload', requireRole([UserRole.CLIENT, UserRole.ADMIN]), singleFile, documentController.uploadDocument.bind(documentController));
    // List documents with pagination and filtering
    router.get('/', documentController.listDocuments.bind(documentController));
    // Get document statistics (admin only)
    router.get('/stats', requireRole([UserRole.ADMIN]), documentController.getStatistics.bind(documentController));
    // Get documents by property ID
    router.get('/property/:propertyId', documentController.getDocumentsByPropertyId.bind(documentController));
    // Get documents by user ID (optional parameter, defaults to current user)
    router.get('/user/:userId?', documentController.getDocumentsByUserId.bind(documentController));
    // Get document by ID
    ...
    // Preview document
    router.get('/:id/preview', documentController.previewDocument.bind(documentController));
    // Delete document
    router.delete('/:id', documentController.deleteDocument.bind(documentController));
    // Soft delete document
    router.patch('/:id/delete', documentController.softDeleteDocument.bind(documentController));
    // Restore soft-deleted document (admin only)
    router.patch('/:id/restore', requireRole([UserRole.ADMIN]), documentController.restoreDocument.bind(documentController));
    return router;
}
/**
 ...
 */
export function mountDocumentsRoutes(app, path = '/documents', prisma) {
