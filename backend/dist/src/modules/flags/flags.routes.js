 ...
 */
export const createFlagsRouter = () => {
    const router = Router();
    // Admin CRUD for flags
    router.get('/admin/flags', authGuard, roleGuard(['ADMIN']), flagsController.getAll);
    router.patch('/admin/flags/:key', authGuard, roleGuard(['ADMIN']), validateParams(FlagKeyParamSchema), validateBody(UpdateFlagSchema), flagsController.update);
    // Client-facing read endpoint
    router.get('/flags', authGuard, flagsController.getAll);
    return router;
};
 ...
 */
export const mountFlagsRoutes = (app, basePath = '/api') => {
