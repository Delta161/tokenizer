...
export class NotificationDispatcherService {
     ...
     */
    constructor(availableChannels, config) {
        this.config = config;
    ...
    async dispatchNotification(user, notification) {
        ...
        this.logger.info(`Dispatching notification ${notification.id} to user ${user.id} via ${this.channels.size} channels`);
        ...
        const dispatchPromises = Array.from(this.channels.values()).map(async (channel) => {
            try {
                ...
                if (!channel.isAvailableFor(user, notification)) {
                    this.logger.info(`Channel ${channel.channelId} not available for user ${user.id} for notification ${notification.id}`);
                    return;
                }
                ...
                this.logger.error(`Unexpected error in channel dispatch for ${channel.channelId}: ${error instanceof Error ? error.message : String(error)}`);
                channelResults.push({
