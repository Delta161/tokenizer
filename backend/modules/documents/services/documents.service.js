...
export class DocumentService {
     ...
     * @returns Array of document records
     */
    async getDocumentsByUserId(targetUserId, requestingUserId) {
     ...
     */
    async listDocuments(filters, page, limit, requestingUserId, includeDeleted = false) {
        // Verify requesting user exists
        const requestingUser = await this.prisma.user.findUnique({
            where: { id: requestingUserId },
        });
     ...
     * @returns Document record and readable stream
     */
    async previewDocument(id, userId) {
    ...
    async getStatistics(userId) {
        ...
        const propertyStats = await this.prisma.$queryRaw `
