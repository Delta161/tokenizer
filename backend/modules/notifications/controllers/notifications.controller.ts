...
import { validateCreateNotification, validateGetNotifications, validateMarkAsRead } from '../validators/notifications.validators';
...
export class NotificationController {
   ...
   */
  async getMyNotifications = async (req: Request, res: Response) => {
    try {
   ...
   * @param res - The Express response object
   */
  async getUnreadCount = async (req: Request, res: Response) => {
    try {
      // Get user ID from session (set by requireAuth middleware)
      const userId = req.user?.id;
   ...
   * @param res - The Express response object
   */
  async markAsRead = async (req: Request, res: Response) => {
    try {
      // Get user ID from session (set by requireAuth middleware)
      const userId = req.user?.id;
   ...
   * @param res - The Express response object
   */
  async markAllAsRead = async (req: Request, res: Response) => {
    try {
      // Get user ID from session (set by requireAuth middleware)
      const userId = req.user?.id;
   ...
   * @param res - The Express response object
   */
  async sendBroadcast = async (req: Request, res: Response) => {
    try {
      ...
      // Send broadcast
      const count = await this.notificationService.sendBroadcast(
        userIds,
        type,
        title,
        message
