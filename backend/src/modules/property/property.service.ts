...
export class PropertyService {
   ...
   */
  async getPropertyById(id: string): Promise<PropertyDTO | null> {
    try {
   ...
   */
  async getApprovedPropertyById(id: string): Promise<PropertyDTO | null> {
    try {
   ...
   */
  async createProperty(clientId: string, data: CreatePropertyDTO, userId?: string): Promise<PropertyDTO> {
    try {
      if (!clientId) {
        throw new PropertyAccessError('Client ID is required');
      }
   ...
   */
  async deleteProperty(id: string, clientId: string | null, userId?: string): Promise<boolean> {
    try {
      // Find the property first to check permissions
      const property = await this.prisma.property.findUnique({ where: { id } });
