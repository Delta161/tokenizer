...
export class InvestmentController {
  ...
  create = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      const investment = await this.investmentService.create(investorId, parse.data as InvestmentCreateDTO);
  ...
  getMyInvestments = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      const investorId = req.user?.id;
      if (!investorId) return res.status(403).json({ success: false, error: 'Forbidden', message: 'Not an investor' });
      const investments = await this.investmentService.getMyInvestments(investorId);
  ...
  getAll = async (req: Request, res: Response): Promise<void> => {
    try {
      const investments = await this.investmentService.getAll(req.query);
      res.json({ success: true, data: investments });
    } catch (error) {
      const errorObj = error as Error;
      res.status(500).json({ success: false, error: errorObj.name || 'Error', message: errorObj.message || 'An unknown error occurred' });
  ...
  getById = async (req: Request, res: Response): Promise<void> => {
    try {
      const investment = await this.investmentService.getById(req.params.id);
  ...
  updateStatus = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const investment = await this.investmentService.updateStatus(id, dto);
      res.json({ success: true, data: investment });
    } catch (error) {
