...
export class DocumentService {
   ...
   */
  async softDeleteDocument(documentId: string, userId: string): Promise<Document> {
    // Verify user exists
    const user = await this.prisma.user.findUnique({
      where: { id: userId },
    });
  ...
  async getStatistics(userId: string): Promise<DocumentStatsDto> {
    ...
    const byPropertyRaw = await this.prisma.$queryRaw<Array<{ propertyId: string, propertyTitle: string, count: BigInt }>>`
      SELECT 
        d."propertyId", 
    ...
    const byUserRaw = await this.prisma.$queryRaw<Array<{ userId: string, userName: string, count: BigInt }>>`
      SELECT 
        d."userId", 
   ...
   */
  async downloadDocument(documentId: string, userId: string): Promise<{ document: Document; stream: Readable }> {
